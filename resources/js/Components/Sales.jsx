    ...
    <>
      <div className='grid grid-cols-4 gap-4'>
        ...
        <div className='relative z-50'>
          <button className='py-2 px-4 font-bold rounded-lg bg-gray-800 text-white w-full dark:text-slate-900' onClick={() => setShowShoppingCart(!showShoppingCart)}>
            <FontAwesomeIcon icon={faCartShopping} /> {addToCart.length > 0 ? "Order Summary" + " (" + addToCart.length + " Item)" : "Cart is empty"}
          </button>
          <Transition show={showShoppingCart} enter='transition ease-out duration-300' enterFrom='opacity-0 scale-95' enterTo='opacity-100 scale-100' leave='transition ease-in duration-75' leaveFrom='opacity-100 scale-100' leaveTo='opacity-0 scale-95'>
            <ShoppingCart cart={addToCart} updateCartQuantities={updatedCartQuantities} updateProductPrice={updateProductPrice} className='absolute z-50 w-[500px] max-h-[60vh] right-0 shadow-lg rounded-xl mt-3' />
          </Transition>
        </div>
      </div>
      <div className='grid grid-cols-5 gap-0 mt-5 justify-between overflow-auto max-h-[90%]'>
        {filterProducts(products, search).map((product) => (
          <div className='group card w-[240px] bg-slate-200 hover:shadow-lg hover:shadow-slate-400 hover:bg-slate-100 p-3 mt-3 rounded-xl cursor-pointer hover:scale-105 transition-all delay-150 duration-300 ease-in-out' key={product.id}>
            <div className='relative'>
              <div className='rounded-lg h-[220px] w-full object-cover bg-center' style={{ backgroundImage: `url("https://source.unsplash.com/random/300×300/?${product.category.name}")` }} aria-hidden='true'></div>
